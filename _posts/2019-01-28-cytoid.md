...
> 2020/2/21 同步修复wiki错误: 无论什么软件,判定线都需要~~0.9s~~0.8s来矫正到对应位置,见[第二步 : 调整音频](#第二步--调整音频)

> 2020/2/21 更规范化的音频剪辑方法,见[第二步 : 调整音频](#第二步--调整音频)
...
> Q : 为什么我们要调整音频?
>
> A : 实际上,Cytoid需要 ~~0.9s~~ 0.8s来调整判定线,如果没有这段时间,note会出现得非常突然
...
> 补充: 如果你觉得这样效果不好,可以直接在源音频前面加 ~~0.9s~~ 0.8s的静默,这样更简单但是可能导致节拍对不上!!! 

> 最好的办法: 增加静默时加0.8s以上的静默,至于加多少?这里有个公式.请把实际BPM代入下方公式计算:

> ```  Result = 0.8 + ( 60 / BPM )*k ``` ,k是任意一个自然数,你可以在自然数中选取一个适合的数代入k
...
由于编者之前已经做过PCTyx使用的wiki翻译,因此这里照搬,并且更正一些翻译
...
PCTyx是JCEXE制作的制谱程序,也是Project Thrynox的后作。该指南已针对Cytus2格式标准进行了更新。
...
> 编者注: 原贴已停止更新,请到[https://github.com/XionUzuki/PCtyx](https://github.com/XionUzuki/PCtyx) 检查更新.
...
> 编者注: 建议隔30min就保存重启,PCtyx在使用时间过长时可能会出现音画不同步的问题
...
如果y轴上的一个页面上有多个锁链，想要锁链不连接，当光标在note上时，按Q或E为每个note分配一个“连接值”.

具有相同值的那些note将被连接，而具有不同值的那些将不会.

请保证同一时刻没有子锁链或锁链头ID相同.当一个锁链头拥有链接值后,它不会再与之前有相同ID的子锁链链接.
...
抱歉,视频在线播放咕了,可以下载下来.[点我下载](/posts-source/cytoid/video/1.mp4)
